...
const CheckoutForm = (props) => {
  ...
  useEffect(() => {
    console.log("CheckoutForm CheckoutForm", props.Order);
  }, [props]);
  ...
  const handleSubmit = async (event) => {
    ...
    if (payload.error) {
      ...
      let OrderData = {
        ...
        productsId: props.Order.productsId,
        prix: props.Order.prix,
        billingId: props.Order.billingId,
        Order_notes: props.Order.Order_notes,
        Order_date: props.Order.Order_date,
        etat: "paye",
      };
      let Payment = {
        user_id:props.Order.userId,
        billing_details: {
      ...
      axios
        .put(
          `http://localhost:3200/api/update_order_By_id/${params.id}`,
          OrderData
        )
        .then(async (res) => {
          const update = await UpdateProduct(OrderData.productsId);
          postPayment(Payment)
        })
  ...
  const postPayment=(Payment)=>{
    axios
    .post(
      `http://localhost:3200/api/add_payment`,
      Payment
    )
    .then(async (res) => {
  ...
  const UpdateProduct = async (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      const filter = await FilterProduct(data[i]);
      newProduct.push(filter);
    }
    axios
      .post("http://localhost:3200/api/update_Product_quantity", newProduct)
      .then((res) => {
        console.log(res.data.message);
      })
      .catch((err) => console.log(err));
        ...
        if (products[i]._id === data._id) {
          if (products[i].category[0] === "fashion") {
            for (let j = 0; j < products[i].variation.length; j++) {
              if (
                products[i].variation[j].color === data.selectedProductColor
              ) {
...
const Checkout = ({ location, cartItems, currency, products, user, Order,  deleteAllFromCart}) => {
  console.log("hereeee props", [Order, products]);
  const callback=()=>{
    deleteAllFromCart()
  }
  return (
    <Elements stripe={stripePromise} options={ELEMENTS_OPTIONS}>
      <CheckoutForm products={products} Order={Order} callback={callback} />
    </Elements>
  );
};
